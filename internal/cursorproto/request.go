...
func populateRunRequest(run protoreflect.Message, request RunRequest) error {
	...
	if err := addRequestedModel(run, request); err != nil {
		return err
	}
...
func addRequestedModel(run protoreflect.Message, request RunRequest) error {
	if request.Model == "" {
		return nil
	}
	requested, err := nestedMessage(run, "requested_model")
	if err != nil {
		return err
	}
	if err := setString(requested, "model_id", request.Model); err != nil {
		return err
	}
	if request.MaxMode {
		if err := setBool(requested, "max_mode", true); err != nil {
			return err
		}
	}
	for _, parameter := range request.Parameters {
		...
		item, err := nestedMessage(requested, "parameters")
		if err != nil {
			return err
		}
		if err := setString(item, "id", parameter.ID); err != nil {
			return err
		}
		if err := setString(item, "value", parameter.Value); err != nil {
			return err
		}
		if err := appendMessage(requested, "parameters", item); err != nil {
			return err
		}
	}
	return setMessage(run, "requested_model", requested)
}
