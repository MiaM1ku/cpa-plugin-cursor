...
func SessionCookie(accountID, accessToken string) string {
	return sessionCookie(accountID, accessToken)
}
...
func IsDashboardURL(target *url.URL) bool {
	if target == nil {
		return false
	}
	if IsDashboardHost(target.Host) {
		return true
	}
	path := target.EscapedPath()
	return strings.HasPrefix(path, "/api/usage-summary") || strings.HasPrefix(path, "/api/dashboard/")
}

func ApplyDashboardAuth(request *http.Request, accountID, accessToken string) {
	if request == nil {
		return
	}
	request.Header.Del("Authorization")
	request.Header.Set("cookie", sessionCookie(accountID, accessToken))
