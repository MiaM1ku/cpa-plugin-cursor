...
func Test_Handler_ManagementStatus_collapses_effort_variants_to_base_ids(t *testing.T) {
	...
	require.Contains(t, string(response.Body), `"id":"claude-fable-5-thinking","disabled":true`)
...
func Test_Handler_ManagementResource_serves_bilingual_shell_without_exposing_auth_data(t *testing.T) {
	...
	require.Contains(t, string(response.Body), `modelHelp: "列出基座和 -thinking。请求用 reasoning_effort；发送为 base-effort 或 base-thinking-effort。忽略 -fast。Max 模式用 max_mode 或 -1m。"`)
	require.Contains(t, string(response.Body), `modelHelp: "Lists base and -thinking. Send reasoning_effort; Cursor gets base-effort or base-thinking-effort. Fast variants are ignored. Max mode is max_mode or -1m."`)
