...
func Test_Handler_ManagementResource_serves_bilingual_shell_without_exposing_auth_data(t *testing.T) {
	...
	require.Contains(t, string(response.Body), `modelHelp: "只列出基座模型。思考强度用 reasoning_effort（none / low / medium / high / xhigh），Max 模式用 max_mode 或 -max / -1m。"`)
	require.Contains(t, string(response.Body), `modelHelp: "Base models only. Send thinking level with reasoning_effort (none / low / medium / high / xhigh). Use max_mode or a -max / -1m suffix for Max mode."`)
